...
document.getElementById("leaderboardHeader").addEventListener("click", function() {
    ...
    if (isOpen) {
        ...
        // Load leaderboard ONLY when opening, and only once per session
...
async function loadLeaderboard() {
    ...
    try {
        ...
        const top5 = [];
        topSnap.forEach(doc => top5.push({ id: doc.id, ...doc.data() }));
        ...
        // --- 5. Compute exact rank using 3 cheap queries ---
        let outrankCount = 0;
        ...
        // C) Equal stars & wins, higher winpct
        const q3 = query(
            collection(db, "leaderboard"),
            where("month", "==", monthKey),
            where("stars", "==", d.stars),
        ...
        // --- 6. Render player row with exact rank ---
        const row = document.createElement("tr");
        row.innerHTML = `
            <td>${
                actualRank === 1 ? "🥇" :
